...
LOGS=/scrapp/sharpton/data/ibdmouse/logs/lastdb
mkdir -p $LOGS
LOG=${LOGS}/${JOB_ID}.${SGE_TASK_ID}.log

uname -a                                       > $LOG 2>&1
echo "****************************"            >> $LOG 2>&1
echo "RUNNING LASTDB WITH $*"                  >> $LOG 2>&1
...
echo "lastdb -Q $FORMAT $OUTPUT $INPUT" >> $LOG 2>&1
lastdb -Q $FORMAT $OUTPUT $INPUT >> $LOG 2>&1
